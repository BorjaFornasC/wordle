...
export class LetterChecker implements ILetterChecker{
    ...
    checkLetters = (game : Game): void => {
        ...
        let numberOfCoincidencesActualTry: number = 0;
        let stringToPaint = "";
        let actualTry = "";
        const MIN_COINCIDENCES : number = 0;
        for (let i = 0; i < MAX_WORD_SIZE; i++) {
    ...
    checkMisplacedLetters(numberOfCoincidencesActualTry: number, numberOfCoincidencesPickedWord: number): string {
        const MAX_COINCIDENCES: number = 2;
        const MIN_DIFFERENCE: number = 1;
        const MIN_COINCIDENCES: number = 1;
